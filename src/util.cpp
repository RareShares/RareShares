...
const char * const BITCOIN_CONF_FILENAME = "rareshares.conf";
const char * const BITCOIN_PID_FILENAME = "raresharesd.pid";
...
bool LogAcceptCategory(const char* category)
{
    if (category != NULL)
    {
        ...
        if (ptrCategory.get() == NULL)
        {
            ...
            // "rareshares" is a composite category enabling all RareShares-related debug output
            if(ptrCategory->count(string("rareshares"))) {
...
#else
    const char* pszModule = "rareshares";
#endif
    if (pex)
        return strprintf(
...
boost::filesystem::path GetDefaultDataDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\RareSharesCore
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\RareSharesCore
    // Mac: ~/Library/Application Support/RareSharesCore
    // Unix: ~/.raresharescore
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "RareSharesCore";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/RareSharesCore";
#else
    // Unix
    return pathRet / ".raresharescore";
#endif
#endif
}
                    ...
                    map<string, vector<string> >& mapMultiSettingsRet)
{
    ...
    if (!streamConfig.good()){
        // Create empty rareshares.conf if it does not excist
    ...
    {
        // Don't overwrite existing settings so command line settings override rareshares.conf
